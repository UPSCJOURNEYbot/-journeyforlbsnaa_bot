...
_HANDLER_STUB_NAMES = ["admin", "ai_quiz", "mix", "mistakes", "pdf_quiz",
                       "poll_quiz", "podcast", "quiz_play", "reports",
                       "scheduling", "setup_wizard", "translation", "weakquiz"]


def _restore_handler_modules():
    """Drop the whole handlers package subtree (stubs + any real sibling
    imported while stubs were present) so later test files rebuild the REAL
    handler chain (prevents import-order-dependent failures)."""
